...
from util.util import game_over, convert_board, play_computer_move
    ...
    def connect(self):
        ...
        if queryset.exists():
            ...
            if results := game_over(
                ...
                return self.send(json.dumps({"type": "game_over", "content": content}))
        ...
        if queryset.exists():
            ...
            if data["type"] == "player_move":
                if not game.turn == game.player_shape:
                    return self.send(
                        json.dumps({"type": "error", "message": "Not player's turn"})
                    )
                move_index = data["move"]
                board = game.board
                game.board = (
                    board[:move_index] + game.player_shape + board[move_index + 1 :]
                )
                ...
                message = json.dumps({"type": "player_move", "content": content})
                self.send(message)
                return
                ...
                if not game.turn == game.computer_shape:
                    return self.send(
                        json.dumps({"type": "error", "message": "Not computer's turn"})
                    )
